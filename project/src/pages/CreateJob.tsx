...
const Conversations: React.FC = () => {
  ...
  const fetchConversations = useCallback(async (silent = false) => {
    ...
    try {
      ...
      let errorMessage = error.response?.data?.message || 'Failed to load conversations';
  ...
  useEffect(() => {
    ...
    socketRef.current.on('connect', () => {
      ...
      socketRef.current?.emit('join_room', selectedChat?.jobId);
    });
    ...
    socketRef.current.on('connect_error', (error) => {
      ...
      setError({
        show: true,
        message: 'Failed to connect to real-time updates. Messages will still work.',
        severity: 'warning',
      });
    });
