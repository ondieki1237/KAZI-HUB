...
import { User, Calendar, MessageSquare, Download, DollarSign } from 'lucide-react';
...
const ViewApplications: React.FC = () => {
  ...
  const handlePaymentClick = (workerId: string) => {
    navigate(`/payment/${jobId}/${workerId}`);
  };
  ...
  // If no pending applications, default to showing all applications
  useEffect(() => {
    if (!loading && job?.applications) {
      const hasPending = job.applications.some((app) => app.status === 'pending');
      if (!hasPending) {
        setFilterStatus('all');
      }
    }
                    ...
                    <div className="flex flex-col gap-2 mt-4">
                      {application.status === 'pending' ? (
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <button
                            onClick={() => handlePaymentClick(application.workerId._id)}
