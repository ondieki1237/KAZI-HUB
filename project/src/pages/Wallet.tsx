...
function Wallet() {
  ...
  const fetchWalletData = async () => {
    try {
      ...
      const response = await axios.get('http://192.168.1.157:5000/api/payments/wallet', {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://192.168.1.157:5000/api/payments/deposit',
        { 
      ...
      const checkPaymentStatus = async (paymentId: string) => {
        try {
          const statusResponse = await axios.get(
            `http://192.168.1.157:5000/api/payments/${paymentId}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(
        'http://192.168.1.157:5000/api/payments/withdraw',
